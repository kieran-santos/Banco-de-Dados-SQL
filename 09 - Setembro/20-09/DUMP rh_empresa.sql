...
insert  into `projetos`(`id_projeto`,`descritivo`,`bonus`) values (1,'Restruturação da rede','20.00'),(2,'Elaboração da PSI','15.00'),(3,'Construção de um novo site','25.00'),(4,'Otimização de custos','10.00');
...
/* Procedure structure for procedure `proc_erase` */

/*!50003 DROP PROCEDURE IF EXISTS  `proc_erase` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `proc_erase`(IN cod_proj int)
BEGIN
IF(cod_proj IN (SELECT id_projeto FROM projetos WHERE id_projeto = cod_proj)) THEN
	IF(cod_proj IN(SELECT id_projeto FROM funcionarios_projetos
			WHERE id_projeto = cod_proj)) THEN		
		SELECT "Projeto com pendencias" AS msg;
	ELSE	
	  DELETE FROM projetos WHERE cod_proj = id_projeto;
	  SELECT "Projeto apagado com sucesso!!" AS msg;
	END IF;
ELSE
 SELECT "Projeto não cadastrado" AS msg;
END IF;
END */$$
DELIMITER ;
